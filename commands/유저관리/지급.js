...
exports.run = async (client, message, args) => {
  ...
  var bonus = args[1];
  if (!bonus) return message.reply("유저에게 지급할 값을 적어주세요!")

  await db.add(`UserCoin.${User.id}`, bonus);
  await db.add(`UserAccumulate.${User.id}`, bonus);
  await db.set(`ChargeCount.${User.id}`, 1);
