...
exports.run = async (client, message, args) => {
  ...
  await db.subtract(`UserWarn.${User.id}`, 1)
  var warn = await db.get(`UserWarn.${User.id}`);

  await message.reply({embed: {color: "RED", description: `경고가 1회 제거되어 ${User}님의 경고는 ${warn}회 입니다.`}})
  await User.send({embed: {color: "RED", description: `경고가 1회 제거되어 ${User}님의 경고는 ${warn}회 입니다.`}})
}
